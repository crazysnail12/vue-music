...
export default {
  ...
  methods: {
    ...
    _triggerPercent(eName) {
      const percent = this._getPercent()
      this.$emit(eName, percent)
    },
    _getPercent() {
    ...
    _offset(offsetWidth) {
      ...
      this.$refs.progressBtn.style[transform] = `translate3d(${Math.max(0, offsetWidth - progressBtnWidth / 2)}px, 0, 0)`
    }
  },

  watch: {
    percent(newPercent) {
      if (newPercent > 0 && !this.touch.initiated && newPercent < 1) {
        const barWidth = this.$refs.progressBar.clientWidth - progressBtnWidth / 2
